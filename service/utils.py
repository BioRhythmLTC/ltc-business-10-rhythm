...
try:  # sklearn metrics used in plotting/evaluation helpers
    from sklearn.metrics import ConfusionMatrixDisplay, confusion_matrix  # type: ignore
except Exception:  # pragma: no cover
...
def get_runs_dir(
    model_alias: Optional[str] = None, run_id: Optional[str] = None
) -> str:
    """Get runs dir.
...
def get_eval_dir(
    ...
    rid = run_id or RUN_ID
    return os.path.join(
        ".", f'eval_out_{os.getenv("SERVICE_PORT", "8080")}', alias, rid
    )
...
def _safe_symlink(target: str, link_name: str) -> None:
    ...
    try:
        ...
        with open(
            os.path.join(os.path.dirname(link_name), "latest.txt"),
            "w",
            encoding="utf-8",
        ) as f:
...
def load_tokenizer_dynamic(model_name: str, prefer_fast: bool = True):
    ...
    if "deberta" in ln:
        try:
            ...
            print(
                f"[Tokenizer] Fast tokenizer unavailable, fallback to DebertaV2Tokenizer: {e_fast}"
            )
    ...
    try:
        print(
            f"[Tokenizer] Using AutoTokenizer ({'fast' if prefer_fast else 'slow'}) for {model_name}"
        )
        return AutoTokenizer.from_pretrained(
            model_name, use_fast=prefer_fast, trust_remote_code=False
        )
...
def has_type(ann, t):
    ...
    """
    return any(
        (tag != "O" and "-" in tag and tag.split("-", 1)[1] == t) for _, _, tag in ann
    )
    ...
    for i, ch in enumerate(stage_chars):
        ...
        if i + 1 < n:
            ...
            if (
                ...
                and b != " "
                and (
                    (_is_cyrillic(a) and _is_latin(b))
                    or (_is_latin(a) and _is_cyrillic(b))
                )
            ):
                out_chars.append(" ")
...
def classify_units(
    unit_counter: Counter, whitelist: Dict[str, List[str]]
...
def build_unit_patterns(whitelist: Dict[str, List[str]]):
    ...
    units_group = (
        "(?:" + "|".join(sorted(unit_forms_regex, key=len, reverse=True)) + ")"
    )

    pattern_volume = re.compile(
        r"\b\d+[\.,]?\d*\s?" + units_group + r"\b", re.IGNORECASE
    )
    pattern_multipack = re.compile(
        r"\b\d+\s*[xх*]\s*\d+[\.,]?\d*\s?" + units_group + r"\b", re.IGNORECASE
    )
...
def noise_stats(texts: List[str], units_group: str) -> Dict[str, int]:
    ...
    for s in texts:
        if re_repeated.search(s):
            counts["repeated"] += 1
        if re_mixed.search(s):
            counts["mixed_scripts"] += 1
        if re_nospace_volume.search(s):
            counts["nospace_volume"] += 1
    return counts
...
def compute_macro_f1(
    ...
    labels: List[str] = ["TYPE", "BRAND", "VOLUME", "PERCENT"],
) -> Dict:
    """Compute macro f1.
...
def compute_metrics_fn(eval_pred):
    ...
    report = compute_macro_f1(
        gold_char_spans, pred_spans, labels=["TYPE", "BRAND", "VOLUME", "PERCENT"]
    )
...
def resolve_model_dir(
    model: str = None, run_id: str = None, prefer: str = "runs_best"
) -> str:
    """Resolve model dir.
...
def load_for_inference(model_dir: str = None, model: str = None, run_id: str = None):
    ...
    target = model_dir or resolve_model_dir(
        model=model, run_id=run_id, prefer="runs_best"
    )
    ...
    try:
        ...
        print(
            f"[Load] Tokenizer load failed for {target}: {e_tok}. Falling back to base {MODEL_NAME}"
        )
    ...
    try:
        ...
        print(
            f"[Load] Model load failed for {target}: {e_m}. Falling back to base {MODEL_NAME}"
        )
...
def predict_one(
    text: str, model_dir: str = None, model: str = None, run_id: str = None
) -> Dict[str, Any]:
    mdl, tok, device = load_for_inference(
        model_dir=model_dir, model=model, run_id=run_id
    )

    # 1) Preprocess input text (confusables, digit-letter fixes, lowercasing, script-boundary spacing)
    norm_text, idx_map = preprocess_text_with_mapping(
    ...
    def id2(i: int) -> str:
        ...
        if isinstance(cfg_map, dict):
            return cfg_map.get(
                i,
                cfg_map.get(
                    str(i), id2label.get(i, "O") if "id2label" in globals() else "O"
                ),
            )
...
def spans_to_business(text: str, spans: List[Tuple[int, int, str]]) -> Dict[str, Any]:
    ...
    if "VOLUME" in first:
        ...
        if m:
            ...
            if re.search(r"миллилитр|мл\.?|ml", seg, re.I):
                unit = "ML"
            elif re.search(r"литр|л\.?|l", seg, re.I):
                unit = "L"
            elif re.search(r"килограмм|кг\.?|kg", seg, re.I):
                unit = "KG"
            elif re.search(r"г(?:р\.?|рамм)", seg, re.I):
                unit = "G"
            elif re.search(r"шт\w*|pcs?\b|pc\b|бутыл|бан", seg, re.I):
                unit = "PCS"
            else:
                unit = None
...
def predict_batch_pp_preloaded(
    ...
    for t in texts:
        nt, imap = preprocess_text_with_mapping(
            t, do_lower=True, apply_translit_map=True
        )
    ...
    for i, (text, nt, imap, pred_ids, offsets) in enumerate(
        zip(texts, norm_texts, idx_maps, pred_ids_batch, offsets_all)
    ):
...
def evaluate_on_validation(val_split, model, tokenizer, label2id, id2label):
    ...
    if "char_bio" not in val_split.columns:
        ...
        val_split["char_bio"] = [
            to_char_bio(s, ann)
            for s, ann in val_split[["sample", "parsed_annotation"]].itertuples(
                index=False
            )
        ]
    ...
    # Compute metrics
    report = compute_macro_f1(
        gold_spans_val, pred_spans_val, labels=["TYPE", "BRAND", "VOLUME", "PERCENT"]
    )
    print("Validation Macro-F1:", f"{report['macro_f1']:.4f}")
    for k in ["TYPE", "BRAND", "VOLUME", "PERCENT"]:
        r = report[k]
        print(
            f"{k:7s}  P={r['precision']:.3f}  R={r['recall']:.3f}  F1={r['f1']:.3f}  support={r['support']}"
        )
    ...
    boundary_errors, type_confusions, missed, false_alarms = (
        [],
        Counter(),
        Counter(),
        Counter(),
    ...
    for i, (g, p) in enumerate(zip(gold_spans_val, pred_spans_val)):
        ...
        for gs in g:
            for ps in p:
                if (
                    gs[2] == ps[2]
                    and not (gs[1] <= ps[0] or ps[1] <= gs[0])
                    and (gs != ps)
                ):
...
def plot_confusion_matrix(
    gold_spans, pred_spans, labels=None, title="Confusion Matrix (entity-level)"
):
    """
...
def token_level_classification_report_v2(model, dataset, id2label, exclude_O=False):
    ...
    for l in target_labels:
        ...
        support = tp + fn
        print(
            f"{l:10s} P={precision:.3f} R={recall:.3f} F1={f1:.3f}  support={support}"
        )
...
def validation_report_bio(
    ...
    if include_O:
        ...
        target_labels = [
            l for l in all_labels_model_order if str(l).startswith(("B-", "I-"))
        ]
    ...
    for lab in target_labels:
        r = per_label[lab]
        print(
            f'{lab:10s} P={r["precision"]:.3f} R={r["recall"]:.3f} F1={r["f1"]:.3f}  support={r["support"]}'
        )

    if plot_cm:
        plt.figure(
            figsize=(max(8, 0.7 * len(target_labels)), max(6, 0.6 * len(target_labels)))
        )
        sns.heatmap(
            cm,
            annot=True,
...
def predict_annotation_for_text(
    text: str, model: str = None, run_id: str = None
) -> List[Tuple[int, int, str]]:
